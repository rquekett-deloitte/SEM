...
|---|---|---|---|
| Update input data to the most recent period and simulate the model from that point | High | Download pipeline built; candidate awaiting review | `Rscript R/update_data.R` downloads the latest ABS (via the Time Series Directory table downloads) and RBA data for every variable with a downloadable source, applies the transformation noted in the workbook's Variables sheet, validates each series against the existing history (`outputs/data_download_validation.csv`: 72 of 74 sources confirmed) and writes `data-raw/Data_updated.xlsx` with 7 new quarters (2025Q1–2026Q3), existing history untouched. Findings: the workbook's Yhdi 1975Q2 cell contains 5 (should be ~13,989 — the downloaded series matches elsewhere); LhrsPub was rebenchmarked by the ABS (4% median, worst 1996Q3); Wfor has recent revisions; Whh now publishes in $bn (auto-rescaled ×1000). Review the candidate, then promote it to `Data.xlsx` and re-run the model. |
...
| Automate the process for updating data inputs, including ensuring that variables currently taken from MasterData are sourced directly in SEM | Medium | Origin, estimation windows and residual conditioning automated; MasterData sourcing needs a data-owner decision | The pipeline now derives the forecast origin, estimation windows and residual conditioning from the data end, so updating `Data.xlsx` is the only manual data step. Source audit of the workbook's `Variables` sheet (`outputs/variable_source_audit.csv`, 2026-09-02): 103 of 122 data columns have a clear source — 92 with official series IDs (ABS/RBA/ATO/Dept of Education), 11 documented exogenous scenario series. The true Masterdata dependency is only 5 variables (PeRatio, EqEarn, LavhMkt, Ustar, DumTsfTot). 8 columns have no documented source at all (BizLns, BizBnd, BizEq, Ygmi, YprpR, YprpP, Peq, KNbiz) and need confirmation from the data owner; 5 more are internal/vague (GovDef, GovDebt, KdepRate, PcpiExGst, ShockGst). Note: `VARIABLES.md` references `data-raw/exogenous_sources.csv`, `outputs/variable_audit.csv` and `outputs/exogenous_assumptions.csv`, none of which exist in the repo. |
