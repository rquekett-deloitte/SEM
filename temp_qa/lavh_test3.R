...
for (type in c("Original")) {
  ...
  q <- as.Date(sprintf("%d-%02d-01", as.integer(format(dates, "%Y")), 3 * ((as.integer(format(dates, "%m")) + 2) %/% 3))); lavh <- tibble(date = q, value = hrs / emp) %>% group_by(date) %>% summarise(value = mean(value), .groups = "drop") %>% filter(is.finite(value))
